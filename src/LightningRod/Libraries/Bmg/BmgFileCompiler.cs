...
public class BmgFileCompiler : IFileCompiler<BmgFile>
{
    ...
    public void Compile(BmgFile file, Stream fileStream)
    {
                ...
                (m1, m2) =>
                {
                    ...
                    if (cmp == 0)
                        throw new InvalidDataException(
                            $"BMG message IDs must be unique. A message with the ID {m1.Id} already exists."
                        );
                    return cmp;
                }
            );
        ...
        if (file.HasStr1)
        {
            file.Messages.Sort(
                (m1, m2) =>
                {
                    ...
                    if (cmp == 0)
                        throw new InvalidDataException(
                            $"BMG message labels must be unique. A message with the label {m1.Label} already exists."
                        );
                    return cmp;
                }
            );
        ...
        if (file.HasMid1)
            WriteMid1(writer, file);
        if (file.HasStr1)
            WriteStr1(writer, file);
    ...
    private static void WriteHeader(FileWriter writer, BmgFile file)
    {
        ...
        if (Equals(file.Encoding, Encoding.GetEncoding(1252)))
        ...
        else if (Equals(file.Encoding, Encoding.GetEncoding("Shift-JIS")))
            writer.Write((byte)0x03);
        else if (Equals(file.Encoding, Encoding.UTF8))
    ...
    private static void WriteInf1Data(
        FileWriter writer,
        BmgFile file,
        int attributeSize,
        uint[] offsets
    ...
    private static void WriteFlw1(FileWriter writer, BmgFile file)
    {
        ...
        var nodeCount =
            file.FlowData!.Nodes.Length + BinaryUtils.GetOffset(file.FlowData.Nodes.Length, 2);
        var labelCount =
            file.FlowData.Labels.Length + BinaryUtils.GetOffset(file.FlowData.Labels.Length, 8);
    ...
    private static void WriteFli1(FileWriter writer, BmgFile file)
    {
        ...
        {
            if (indexData.Length > 8)
                writer.Write(indexData, 0, 8);
            else
            {
