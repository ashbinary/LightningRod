...
public static class DataTypes
{
    ...
    public static DataType Bool { get; } =
        new()
        {
            Name = "bool",
            Length = 1,
            Serialize = (str, _, _) =>
                "true".Equals(str, StringComparison.OrdinalIgnoreCase) ? [1] : [0],
    ...
    public static DataType Byte { get; } =
        new()
        {
            Name = "u8",
            Length = 1,
    ...
    public static DataType SByte { get; } =
        new()
        {
            Name = "s8",
            Length = 1,
    ...
    public static DataType Int16 { get; } =
        new()
        {
            Name = "s16",
            Length = 2,
            Serialize = (str, isBigEndian, _) =>
                GetBytes(BitConverter.GetBytes(short.Parse(str)), isBigEndian),
            Deserialize = (bytes, offset, isBigEndian, _) =>
                (BitConverter.ToInt16(GetBytes(bytes, offset, 2, isBigEndian)).ToString(), 2),
        };
    ...
    public static DataType UInt16 { get; } =
        new()
        {
            Name = "u16",
            Length = 2,
            Serialize = (str, isBigEndian, _) =>
                GetBytes(BitConverter.GetBytes(ushort.Parse(str)), isBigEndian),
            Deserialize = (bytes, offset, isBigEndian, _) =>
                (BitConverter.ToUInt16(GetBytes(bytes, offset, 2, isBigEndian)).ToString(), 2),
        };
    ...
    public static DataType Int32 { get; } =
        new()
        {
            ...
            Length = 4,
            Serialize = (str, isBigEndian, _) =>
                GetBytes(BitConverter.GetBytes(int.Parse(str)), isBigEndian),
            Deserialize = (bytes, offset, isBigEndian, _) =>
                (BitConverter.ToInt32(GetBytes(bytes, offset, 4, isBigEndian)).ToString(), 4),
        };
    ...
    public static DataType UInt32 { get; } =
        new()
        {
            Name = "u32",
            Length = 4,
            Serialize = (str, isBigEndian, _) =>
                GetBytes(BitConverter.GetBytes(uint.Parse(str)), isBigEndian),
            Deserialize = (bytes, offset, isBigEndian, _) =>
                (BitConverter.ToUInt32(GetBytes(bytes, offset, 4, isBigEndian)).ToString(), 4),
        };
    ...
    public static DataType Int64 { get; } =
        new()
        {
            ...
            Length = 8,
            Serialize = (str, isBigEndian, _) =>
                GetBytes(BitConverter.GetBytes(long.Parse(str)), isBigEndian),
            Deserialize = (bytes, offset, isBigEndian, _) =>
                (BitConverter.ToInt64(GetBytes(bytes, offset, 8, isBigEndian)).ToString(), 8),
        };
    ...
    public static DataType UInt64 { get; } =
        new()
        {
            Name = "u64",
            Length = 8,
            Serialize = (str, isBigEndian, _) =>
                GetBytes(BitConverter.GetBytes(ulong.Parse(str)), isBigEndian),
            Deserialize = (bytes, offset, isBigEndian, _) =>
                (BitConverter.ToInt64(GetBytes(bytes, offset, 8, isBigEndian)).ToString(), 8),
        };
    ...
    public static DataType Single { get; } =
        new()
        {
            Name = "f32",
            Length = 4,
            Serialize = (str, isBigEndian, _) =>
                GetBytes(
                    BitConverter.GetBytes(float.Parse(str, CultureInfo.InvariantCulture)),
                    isBigEndian
                ),
            Deserialize = (bytes, offset, isBigEndian, _) =>
                (
                    BitConverter
                        .ToSingle(GetBytes(bytes, offset, 4, isBigEndian))
    ...
    /// </summary>
    [TypeName("f64", "double")]
    public static DataType Double { get; } =
        new()
        {
            Name = "f64",
            Length = 8,
            Serialize = (str, isBigEndian, _) =>
                GetBytes(
                    BitConverter.GetBytes(double.Parse(str, CultureInfo.InvariantCulture)),
                    isBigEndian
                ),
            Deserialize = (bytes, offset, isBigEndian, _) =>
                (
                    BitConverter
                        .ToDouble(GetBytes(bytes, offset, 8, isBigEndian))
                        .ToString(CultureInfo.InvariantCulture),
                    8
                ),
        };
    ...
    /// </summary>
    [TypeName("str", "string")]
    public static DataType String { get; } =
        new()
        {
            Name = "str",
            Length = 2,
    ...
    public static DataType NullString { get; } =
        new()
        {
            Name = "nstr",
            Length = 1,
            Serialize = (str, _, encoding) => encoding.GetBytes(str + '\0'),
            Deserialize = (bytes, offset, _, encoding) =>
                ReadTerminatedString(encoding, bytes, offset),
        };
    ...
    public static DataType HexString { get; } =
        new()
        {
            Name = "hex",
            Length = 0,
...
#else
                return (
                    BitConverter.ToString(bytes, offset).Replace("-", string.Empty),
                    bytes.Length - offset
                );
#endif
            },
    ...
    public static IReadOnlyDictionary<string, DataType> GetTypeList()
    {
        lock (Lock)
        {
            ...
            foreach (
                var field in typeof(DataTypes).GetProperties(
            ...
            )
            {
                ...
                if (attr is null)
                    cache.TryAdd(field.Name, type);
                else
                    foreach (var name in attr.Names)
                        cache.TryAdd(name, type);
            }
    ...
    private static (string, int) ReadTerminatedString(Encoding encoding, byte[] data, int offset)
    {
        ...
        for (var i = 0; i < nullByteLength; ++i)
            bytes.Add(0x00);
    ...
    private static byte[] GetBytes(byte[] data, bool isBigEndian) =>
        GetBytes(data, 0, data.Length, isBigEndian);
