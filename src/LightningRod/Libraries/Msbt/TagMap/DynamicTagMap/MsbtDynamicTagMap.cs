...
public class MsbtDynamicTagMap : IMsbtTagMap, IEnumerable<MsbtTagInfo>
{
    ...
    private readonly Dictionary<string, MsbtTagInfo> _namedLookupMap = new(StringComparer.OrdinalIgnoreCase);
    #endregion
    ...
    public void GetTag(MsbtTag tag, bool bigEndian, Encoding encoding, out string tagName, out IEnumerable<MsbtTagArgument> tagArgs)
    {
        ...
        if (TryGetInfo(tag.Group, tag.Type, out var tagInfo))
        {
            ...
            {
                if (tagInfo.TypeMap.TryGetValue(tag.Type.ToString(), out var typeName)) tagName += $":{typeName.NameOrValue}";
                else tagName += $":{tag.Type}";
            }
            ...
            foreach (var argInfo in tagInfo.Arguments)
            {
                ...
                {
                    argOffset += argInfo.DataType.Length * (argInfo.ArrayLength > 0 ? argInfo.ArrayLength : 1);
                    continue;
                }
            ...
            {
                argList.Add(new MsbtTagArgument("otherArg", tag.Args[argOffset..].ToHexString(true)));
            }
        }
        else
        {
        ...
        string ParseArgument(MsbtArgumentInfo arg, MsbtTagInfo info)
        {
            try
            {
                var (value, count) = arg.DataType.Deserialize(tag.Args, argOffset, bigEndian, encoding);
                argOffset += count;
                return arg.ValueMap.TryGetValue(value, out var valueInfo) ? valueInfo.NameOrValue : value;
            }
            catch
            {
                throw new FormatException($"Failed to parse tag argument value of \"{arg.Name}\" on \"{info.Name}\" as {arg.DataType.Name}.");
            }
        }
    }
    #endregion
    ...
    public void AddInfo(MsbtTagInfo tag)
    {
        ...
        //nothing to add
        if (!tag.Type.HasValue && tag is {HasDiscard: false, TypeList.Count: 0, TypeMap.Count: 0}) return;

        //check names and types
        if (!string.IsNullOrEmpty(tag.Name))
        {
            if (tag.Name.ContainsAny(':', ' ')) throw new ArgumentException("Tag name contains invalid characters.");
            if (ushort.TryParse(tag.Name, out _)) throw new ArgumentException("Tag name cannot be a number.");
        }
        if (tag.TypeMap.Count > 0 && tag.TypeMap.DataType != DataTypes.UInt16) throw new ArgumentException("Tag type map datatype must be u16.");
        foreach (var type in tag.TypeMap)
        {
            if (string.IsNullOrEmpty(type.Name)) continue;
            if (type.Name.ContainsAny(':', ' ')) throw new ArgumentException("Tag type name contains invalid characters.");
            if (ushort.TryParse(type.Name, out _)) throw new ArgumentException("Tag type name cannot be a number.");
        }
        foreach (var argument in tag.Arguments)
        {
            if (argument.ValueMap.Count == 0) continue;
            if (argument.IsDiscard) throw new ArgumentException("Tag argument cannot be discard and value map at the same time.");
            if (argument.DataType != argument.ValueMap.DataType) throw new ArgumentException($"Tag argument value map datatype must be {argument.DataType.Name}.");
        }

        //check for value collisions
        if (_numericLookupMap.TryGetValue(tag.Group, out var tagGroup))
        {
            if (tag.HasDiscard && tagGroup.Discard is not null) throw new ArgumentException("A discard tag already exists in the same group.");
            if (tag.Type.HasValue)
            {
                if (tagGroup.Tags.ContainsKey(tag.Type.Value)) throw new ArgumentException($"A tag with the type {tag.Type.Value} already exists in the same group.");
                if (!string.IsNullOrEmpty(tag.Name) && _namedLookupMap.ContainsKey(tag.Name)) throw new ArgumentException($"A tag with the name {tag.Type.Value} already exists in the same group.");
            }
            foreach (var type in tag.TypeList)
            {
                if (tagGroup.Tags.ContainsKey(type)) throw new ArgumentException($"A tag with the type {type} already exists in the same group.");
            }
            foreach (var typeInfo in tag.TypeMap)
            {
                var typeId = ushort.Parse(typeInfo.Value);
                if (tagGroup.Tags.ContainsKey(typeId)) throw new ArgumentException($"A tag with the type {typeId} already exists in the same group.");
                if (!string.IsNullOrEmpty(typeInfo.Name) && _namedLookupMap.ContainsKey($"{tag.Group}:{typeInfo.Name}")) throw new ArgumentException($"A tag with the name {typeInfo.Name} already exists in the same group.");
            }
        }
        ...
        if (tag.HasDiscard)
        {
            ...
            if (!string.IsNullOrEmpty(tag.Name)) _namedLookupMap.Add($"{tag.Name}::", tag);
        }
        foreach (var type in tag.TypeList)
        {
            tagGroup.Tags.Add(type, tag);
            _namedLookupMap.Add($"{tag.Group}:{type}", tag);
            if (!string.IsNullOrEmpty(tag.Name)) _namedLookupMap.Add($"{tag.Name}:{type}", tag);
        }
        foreach (var typeInfo in tag.TypeMap)
        {
            tagGroup.Tags.Add(ushort.Parse(typeInfo.Value), tag);
            _namedLookupMap.Add($"{tag.Group}:{typeInfo.Value}", tag);
            if (!string.IsNullOrEmpty(typeInfo.Name)) _namedLookupMap.Add($"{tag.Group}:{typeInfo.Name}", tag);
            if (!string.IsNullOrEmpty(tag.Name))
            {
                _namedLookupMap.Add($"{tag.Name}:{typeInfo.Value}", tag);
                if (!string.IsNullOrEmpty(typeInfo.Name)) _namedLookupMap.Add($"{tag.Name}:{typeInfo.Name}", tag);
            }
        }
    }
    ...
    public bool TryGetInfo(ushort group, ushort type, [MaybeNullWhen(false)] out MsbtTagInfo tag)
    {
        ...
        if (!_numericLookupMap.TryGetValue(group, out var tagGroup)) return false;
        if (tagGroup.Tags.TryGetValue(type, out tag)) return true;
    ...
    {
        if (_namedLookupMap.TryGetValue(tagName, out tag)) return true;
        var tagParts = tagName.Split(':');
        if (tagParts.Length != 2 || !ushort.TryParse(tagParts[1], out _)) return false;
