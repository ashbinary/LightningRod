...
public static class DynamicTagMapParser
{
    ...
    private static readonly IReadOnlyDictionary<string, DataType> DatatypeMap = DataTypes.GetTypeList();
    private static readonly Regex TagRegex = new(@"^\[\s*(\d+)\s*,\s*(\d+|\d+\s*-\s*\d+|_|\(\s*\d+(?:\s*-\s*\d+)?(?:\s*,\s*\d+(?:\s*-\s*\d+)?)*\s*\)|\{[A-Za-z0-9_]+\})\s*\]\s*(?:\s+([A-Za-z0-9_]+))?\s*(?:\s+#\s*(.*))?$");
    private static readonly Regex ArgumentRegex = new(@"^\s{2,}([A-Za-z0-9_]+|\{[A-Za-z0-9_]+\})(?:\[(\d+)\])?\s*(?:\s+([A-Za-z0-9_]+))?\s*(?:\s+#\s*(.*))?$");
    ...
    private static readonly Regex MapRegex = new(@"^map\s+([A-Za-z0-9_]+)(?:\s+([A-Za-z0-9_]+))?\s*(?:\s+#\s*(.*))?$");
    private static readonly Regex MapValueRegex = new(@"^\s{2,}(-?[A-Za-z0-9_]+)(?:\s+([A-Za-z0-9_]+))?\s*(?:\s+#\s*(.*))?$");
    #endregion
    ...
    public static MsbtDynamicTagMap ParseMsbtTagMap(string content)
    {
        ...
        foreach (var (line, tag) in ParseMap(content, false))
        {
            ...
            {
                throw new FormatException($"Failed to add tag definition to map: {ex.Message} (line {line + 1})");
            }
        }
        return map;
    }
    ...
    private static Dictionary<int, TagEntry> ParseMap(string content, bool forBmg)
    {
        ...
        for (var i = 0; i < lines.Length; ++i)
        {
            ...
            if (TagRegex.IsMatch(line, out var tagMatch))
            {
                ...
                if (forBmg)
                {
                    if (!byte.TryParse(tagMatch.Groups[1].Value, out var groupVal)) throw new FormatException($"Failed to parse group ID as 8 bit integer (line {line + 1}).");
                    group = groupVal;
                }
                else if (!ushort.TryParse(tagMatch.Groups[1].Value, out group)) throw new FormatException($"Failed to parse group ID as 16 bit integer (line {line + 1}).");

                if (!isDiscardType && !TryGetTypeList(tagMatch.Groups[2].Value, out typeList)
                                   && !TryGetMapName(tagMatch.Groups[2].Value, out mapName)
                                   && !ushort.TryParse(tagMatch.Groups[2].Value, out type))
                {
                    throw new FormatException($"Failed to parse type ID as 16 bit integer (line {line + 1}).");
                }

                currentTag = new TagEntry
                {
                    Group = group,
                    Type = isDiscardType || typeList.Length > 0 || mapName is not null ? null : type,
                    TypeList = typeList,
                    HasDiscard = isDiscardType,
                    Name = tagMatch.Groups[3].Success ? tagMatch.Groups[3].Value : string.Empty,
                    Description = tagMatch.Groups[4].Success ? tagMatch.Groups[4].Value.Trim() : string.Empty
                };
                tags.Add(i, currentTag);

                if (mapName is not null) mapReferences.Add(i, (currentTag, mapName));
            ...
            {
                if (discardDefined) throw new FormatException($"Argument discard padding must be the last argument in a tag declaration (line {i+1}).");

                var dataType = DataTypes.Bool;
                var isPadding = false;
                if (!TryGetMapName(argumentMatch.Groups[1].Value, out var mapName) && !DatatypeMap.TryGetValue(argumentMatch.Groups[1].Value, out dataType))
                {
                    ...
                    {
                        if (argumentMatch.Groups[2].Success) throw new FormatException($"Argument discard padding cannot be an array (line {i+1}).");

                        dataType = DataTypes.GetPadding(discardMatch.Groups[1].Success ? discardMatch.Groups[1].Value : "0x00");
                        isPadding = true;
                        discardDefined = true;
                    }
                    else throw new FormatException($"Unknown argument data type \"{argumentMatch.Groups[1].Value}\" on line {i+1}.");
                }

                var arrayLength = 0;
                if (argumentMatch.Groups[2].Success && !int.TryParse(argumentMatch.Groups[2].Value, out arrayLength)) throw new FormatException($"Failed to parse array length as 32 bit integer (line {i+1}).");

                var name = argumentMatch.Groups[3].Success ? argumentMatch.Groups[3].Value : $"arg{currentTag!.Arguments.Count + 1}";
                ...
                var arg = new ArgumentEntry
                {
                    ...
                    Name = name,
                    Description = argumentMatch.Groups[4].Success ? argumentMatch.Groups[4].Value.Trim() : string.Empty
                };

                if (!currentTag!.Arguments.TryAdd(arg, a => a.DataType is PaddingDataType && isPadding || a.Name != name))
                {
                    throw new FormatException($"An argument declaration with the same name already exists ({i+1}).");
                }
            ...
            if (MapRegex.IsMatch(line, out var mapMatch))
            {
                ...
                if (mapMatch.Groups[2].Success && !DatatypeMap.TryGetValue(mapMatch.Groups[2].Value, out dataType))
                {
                    throw new FormatException($"Unknown map data type \"{mapMatch.Groups[2].Value}\" on line {i+1}.");
                }

                currentValueMap = ([], dataType);

                if (!valueMaps.TryAdd(mapMatch.Groups[1].Value, ((List<MsbtValueInfo>, DataType)) currentValueMap))
                {
                    throw new FormatException($"A map declaration with the same name already exists (line {i+1}).");
                }
            ...
            if (mapSection && MapValueRegex.IsMatch(line, out var mapValueMatch))
            {
                ...
                var name = mapValueMatch.Groups[2].Success ? mapValueMatch.Groups[2].Value : string.Empty;

                var datatype = currentValueMap!.Value.Item2;
                string valueStr;
                try
                {
                    var valueBytes = datatype.Serialize(value, !BitConverter.IsLittleEndian, Encoding.UTF8);
                    valueStr = datatype.Deserialize(valueBytes, 0, !BitConverter.IsLittleEndian, Encoding.UTF8).Item1;
                }
                catch
                {
                    throw new FormatException($"Failed to convert map value \"{value}\" to \"{datatype.Name}\" (line {i+1}).");
                }
                ...
                var item = new MsbtValueInfo
                {
                    Value = valueStr,
                    Name = name,
                    Description = mapValueMatch.Groups[3].Success ? mapValueMatch.Groups[3].Value.Trim() : string.Empty
                };

                if (!currentValueMap.Value.Item1.TryAdd(item, v => v.Value != value && (string.IsNullOrEmpty(name) || v.Name != name)))
                {
                    throw new FormatException($"A map item with the same name or value already exists ({i+1}).");
                }

                continue;
            }

            if (tagSection) throw new FormatException($"Invalid argument format on line {i+1}.");
            if (mapSection) throw new FormatException($"Invalid map item format on line {i+1}.");
        ...
        foreach (var (line, (reference, mapName)) in mapReferences)
        {
            ...
            {
                throw new FormatException($"Value map \"{mapName}\" is not defined (line {line + 1}).");
            }
            ...
            {
                throw new FormatException($"Failed to parse value map \"{mapName}\" (line {line + 1}): {ex.Message}");
            }

            switch (reference)
            {
                case TagEntry when valueMap.DataType != DataTypes.UInt16:
                    throw new FormatException($"Value maps used for tag types must be \"u16\" (line {line + 1}).");
                case TagEntry tag:
    ...
    private class TagEntry
    {
        ...
        public BmgTagInfo ToBmgInfo() => new()
        {
            Group = (byte) Group,
            Type = Type,
            TypeList = TypeList,
        ...
        public MsbtTagInfo ToMsbtInfo() => new()
        {
            Group = Group,
            Type = Type,
            TypeList = TypeList,
    ...
    private class ArgumentEntry
    {
        ...
        public MsbtArgumentInfo ToMsbtInfo() => new()
        {
            DataType = DataType,
