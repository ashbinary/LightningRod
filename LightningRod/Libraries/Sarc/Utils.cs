...
namespace LightningRod.Libraries.Sarc {
    public static class Utils {
        public static ref T AsStruct<T>(this Span<byte> data) where T : unmanaged
        {
            return ref MemoryMarshal.Cast<byte, T>(data[..Unsafe.SizeOf<T>()])[0];
        }

        public static Span<T> AsStructSpan<T>(this Span<byte> data, int count) where T : unmanaged
        {
            return MemoryMarshal.Cast<byte, T>(data[..(count * Unsafe.SizeOf<T>())]);
        }

        public static T DivideAndCeil<T>(T numerator, T denominator) where T : INumber<T>, IModulusOperators<T, T, T>
        {
        ...
        public static T RoundUp<T>(T value, T alignment) where T : INumber<T>, IModulusOperators<T, T, T>
        {
